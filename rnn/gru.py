...
#     GatedRecurrent     (emb_d * block_size, n_hidden_rnn, n_hidden_l, num_layers=2, dropout=0.2),
...
class SelectLastOutput(nn.Module):
    def __init__(self):
        super(SelectLastOutput, self).__init__()

    def forward(self, x):
        # x is expected to be the output from GRU, which is a tuple (output, hidden)
        # We select the output at the last timestep
        return x[0]#[0][:, -1, :]  # Assuming batch_first=True

model = torch.nn.Sequential(
    torch.nn.Embedding(vocab_size, emb_d),
    torch.nn.Flatten(),
    # GatedRecurrent     (emb_d * block_size, n_hidden_rnn, n_hidden_rnn,),
    # torch.nn.GRU(emb_d * block_size, n_hidden_rnn, num_layers=1, dropout=0.0,), SelectLastOutput(),
    torch.nn.RNN(emb_d * block_size, n_hidden_rnn, num_layers=2, dropout=0.0), SelectLastOutput(),
    torch.nn.Linear(n_hidden_rnn, vocab_size)
)
...
def split_loss(split, do_print=False):
    ...
    for x_b, y_b in val_dataloader:
        # model[2].reset_hidden(val_batch_size)
...
for i in range(num_epochs):
    ...
    for j, (data, targets) in enumerate(train_dataloader):
        ...
        # forward pass
        # model[2].reset_hidden(train_batch_size)
